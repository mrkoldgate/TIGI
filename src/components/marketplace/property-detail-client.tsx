...
function AiValuationPanel({ listing }: { listing: MockListing }) {
  ...
  const deltaColor = delta.variant === 'under' ? '#4ADE80' : delta.variant === 'over' ? '#FCD34D' : '#A0A0B2'

  return (
    <div className="rounded-xl border border-[#2A2A3A] bg-[#0D0D14] p-4">
      <div className="mb-3 flex items-center gap-2">
        <TrendingUp className="h-3.5 w-3.5 text-[#C9A84C]" />
        <span className="text-[11px] font-semibold uppercase tracking-wider text-[#A0A0B2]">
          AI Valuation
        </span>
        <span className="ml-auto text-[10px] text-[#4A4A60]">Beta</span>
      </div>

      <div className="mb-3 flex items-baseline justify-between gap-2">
        <div>
          <p className="text-[10px] text-[#4A4A60]">Estimated value</p>
          <p className="font-heading text-xl font-semibold text-[#F5F5F7]">
            ${listing.aiEstimatedValue.toLocaleString()}
          </p>
        </div>
        <p className="text-right text-xs font-medium" style={{ color: deltaColor }}>
          {delta.label}
        </p>
      </div>

      {/* Confidence bar */}
      <div className="mb-1 flex items-center justify-between text-[10px] text-[#4A4A60]">
      ...
      <div className="h-1 overflow-hidden rounded-full bg-[#2A2A3A]">
        <div
          ...
          style={{ width: confBarWidth, backgroundColor: confColor }}
        />
      </div>

      <p className="mt-3 text-[10px] leading-relaxed text-[#4A4A60]">
        Estimate generated by TIGI AI using comparable sales, market trends, and property attributes. Not a certified appraisal.
      </p>
    </div>
  )
}
